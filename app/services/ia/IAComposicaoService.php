...
class IAComposicaoService
{
    ...
    public function enfileirarBanner(array $entrada, array $tipo): array
    {
        ...
        // Barra ANTES de gastar. Sem Imagick o pipeline só descobriria no
        // último passo, depois de já ter pago o recorte e a cena.
        if (!IACompositorService::disponivel()) {
            return ['ok' => false, 'msg' => 'Composição indisponível: a extensão Imagick não está instalada neste servidor.'];
        }
        ...
        // criar() devolve int: id, 0 em erro ou -1062 na dedup. Comparar com
        // null nunca casava — a duplicata virava sucesso com id -1062 e uuid
        // fantasma, e a tela ficava em polling por uma geração inexistente.
        if ($id === -1062) {
            return ['ok' => false, 'msg' => 'Banner idêntico já solicitado neste minuto — aguarde ou mude o briefing.'];
        }
        if ($id <= 0) {
            return ['ok' => false, 'msg' => 'Erro ao enfileirar o banner.'];
        }
    ...
    public function publicarBanner(int $geracaoId, int $usuarioId, array $dados = []): array
    {
        ...
        // Destino público do arquivo.
        $dirPadrao  = rtrim(dirname(__DIR__, 3), '/') . '/uploads/banners';
        $usouPadrao = !defined('IA_BANNER_DIR') || IA_BANNER_DIR === '';
        $dir        = $usouPadrao ? $dirPadrao : rtrim((string) IA_BANNER_DIR, '/');
        ...
        // banners.arquivo_imagem entra CRU no src: views/partials/banner-item.php
        // chama View::uploadR2($valor), que devolve o valor como veio. Gravar só
        // o nome do arquivo produziria <img src="ia-xxx.jpg">, resolvido em
        // relação à página atual — quebraria em tudo que não fosse a raiz.
        // Por isso o padrão é raiz-relativo; com IA_BANNER_DIR customizado não
        // dá para deduzir a URL, então o prefixo tem de ser declarado.
        $prefixo = (defined('IA_BANNER_VALOR_PREFIXO') && IA_BANNER_VALOR_PREFIXO !== '')
            ? rtrim((string) IA_BANNER_VALOR_PREFIXO, '/') . '/'
            : ($usouPadrao ? '/uploads/banners/' : '');

        if ($prefixo === '') {
            LogService::warning('ia_banner_prefixo_indefinido', [
                'geracao_id' => $geracaoId,
                'dir'        => $dir,
                'aviso'      => 'IA_BANNER_DIR customizado sem IA_BANNER_VALOR_PREFIXO — o src do banner pode não resolver.',
            ]);
        }

        $valorArquivo = $prefixo . $nomeArq;
