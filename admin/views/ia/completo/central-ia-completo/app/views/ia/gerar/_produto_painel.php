...
<?php endif; ?>
    ...
    <div class="ia_form_linha">
      ...
      <div class="ia_form_grupo" id="ia_g_layout_wrap" style="display:none">
        ...
        <select name="layout" id="ia_g_layout">
          <?php foreach (($layouts ?? []) as $l): ?>
            <option value="<?= ia_e($l['codigo']) ?>"><?= ia_e($l['nome']) ?></option>
          <?php endforeach; ?>
        </select>
        <input type="text" name="banner_headline" maxlength="80" placeholder="Headline (vazio = nome do produto)" style="margin-top:8px">
        <input type="text" name="banner_subtitulo" maxlength="120" placeholder="Subtítulo (opcional)" style="margin-top:8px">
        <p class="ia_ajuda">Foto real recortada + cena de IA + preço, montados no formato exato do layout.</p>
      </div>
    </div>
