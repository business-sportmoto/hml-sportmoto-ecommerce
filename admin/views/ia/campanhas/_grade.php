          ...
          <td>
            ...
            <?php elseif ($g['status'] === 'concluida'): ?>
              <button type="button" class="ia_pill ia_pill_ok ia_grade_celula" data-gid="<?= (int) $g['id'] ?>">
                <?= IconLibrary::render('check-circle', 'ia_ico', ['aria-hidden' => 'true']) ?> OK<?php
                  // Curadoria aprovada ganha um segundo selo na propria celula.
                  if ($g['aprovacao'] === 'aprovado') {
                      echo ' ' . IconLibrary::render('shield-check', 'ia_ico', ['aria-hidden' => 'true']);
                  } ?>
              </button>
            <?php elseif ($g['status'] === 'falhou'): ?>
              <button type="button" class="ia_pill ia_pill_erro ia_grade_celula" data-gid="<?= (int) $g['id'] ?>">
                <?= IconLibrary::render('x-circle', 'ia_ico', ['aria-hidden' => 'true']) ?> Falhou
              </button>
            <?php elseif ($g['status'] === 'na_fila'): ?>
              <button type="button" class="ia_pill ia_pill_neutra ia_grade_celula" data-gid="<?= (int) $g['id'] ?>">
                <?= IconLibrary::render('relogio', 'ia_ico', ['aria-hidden' => 'true']) ?> Na fila
              </button>
            <?php else: ?>
