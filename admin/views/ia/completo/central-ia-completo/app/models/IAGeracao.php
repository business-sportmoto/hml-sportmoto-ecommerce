...
class IAGeracao
{
    ...
    public function listarAguardando(int $limite = 5, int $idadeMinSegundos = 20): array
    {
        try {
            ...
            $stmt = $this->db->query(
                "SELECT id, uuid, usuario_id, produto_id, capacidade, formato, etapa, external_id,
    ...
    /** Etapa do pipeline de composição (recorte | cena | null ao concluir). */
    ...
    /** Regrava o contexto (custo acumulado / modelos tentados do pipeline). */
        ...
        try {
            ...
            $stmt = $this->db->prepare(
                "SELECT g.uuid, g.id, g.status, g.aprovacao, g.resultado_texto, g.erro,
                        ...
                        g.tempo_ms, g.angulo, g.criado_em, g.capacidade, g.formato, g.etapa,
