...
try {
    ...
    while ((time() - $inicio) < $loopSegundos) {
        ...
        // Campanhas em lote (Fase 3): o driver enfileira no ritmo; o lote abaixo já pega
        $log((new IACampanhaService())->processarCampanhas());
        ...
        foreach ($lote as $g) {
            try {
                ...
                if (($g['capacidade'] ?? '') === 'composicao') {
                    $log('composicao: ' . (new IAComposicaoService())->processar($g) . " — geração #{$g['id']}");
                    $processadas++;
                    continue;
                }

                $r = in_array(($g['capacidade'] ?? 'texto'), ['imagem', 'remocao_fundo'], true)
