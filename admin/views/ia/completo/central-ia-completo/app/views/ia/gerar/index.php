...
jQuery(function ($) {
  ...
  // ângulo por proporção; composição (banner) troca ambos pelo layout.
  $(document).on('change', '#ia_g_tipo', function () {
    var cap = $(this).find('option:selected').data('cap') || 'texto';
    var ehImagem = (cap === 'imagem');
    var ehComp   = (cap === 'composicao');
    $('#ia_g_angulo_wrap').toggle(!ehImagem && !ehComp);
    $('#ia_g_proporcao_wrap').toggle(ehImagem);
    $('#ia_g_layout_wrap').toggle(ehComp);
    if (ehImagem || ehComp) { $('#ia_g_angulo').val(''); }
  });
  ...
  // Publicar a arte final como banner (nasce INATIVO — revisão na tela de banners)
  $(document).on('click', '.ia_ac_publicar', function () {
    var $b = $(this).prop('disabled', true);
    iaPost(URLS.publicar, { geracao_id: $b.data('id') }, function (r) {
      if (!r.ok) { $b.prop('disabled', false); toast(r.msg || 'Erro ao publicar.', false); return; }
      toast(r.msg, true);
      $b.html('<i class="bi bi-check2"></i> Publicado');
    });
  });
  ...
  function atualizarCard(g) {
    ...
    if (g.status === 'na_fila') {
      ...
      var etapaTxt = (g.capacidade === 'composicao' && g.etapa)
        ? ' (' + ({recorte: 'recortando produto', cena: 'criando cena'}[g.etapa] || g.etapa) + ')' : '';
      $pill.attr('class', 'ia_pill ia_pill_azul').html('<span class="ia_spin"></span> Gerando…' + etapaTxt);
      ...
      if (!$corpo.data('pronto')) {
        if ((g.capacidade === 'imagem' || g.capacidade === 'composicao') && g.arquivo_id) {
          var publicarBtn = (g.capacidade === 'composicao')
            ? '<button type="button" class="ia_btn ia_btn_primario ia_ac_publicar" data-id="' + g.id + '"><i class="bi bi-megaphone"></i> Publicar banner</button>'
            : '';
